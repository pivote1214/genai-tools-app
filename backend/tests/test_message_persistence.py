...
def test_message_persistence_property(role, content, model):
    ...
    **Validates: Requirements 3.1, 3.2**
    ...
    assert messages[0].role == role, f"roleが一致しない: expected={role}, actual={messages[0].role}"
    assert messages[0].content == content, f"contentが一致しない"
    assert messages[0].model == model, f"modelが一致しない: expected={model}, actual={messages[0].model}"
