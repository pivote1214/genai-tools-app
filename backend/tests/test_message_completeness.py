...
def test_saved_message_completeness_property(role, content, model):
    ...
    **Validates: Requirements 3.3**
    ...
    assert saved_message.role == role, f"roleが一致しない: expected={role}, actual={saved_message.role}"
    assert saved_message.role in ['user', 'assistant'], f"roleが不正な値: {saved_message.role}"
    ...
    assert saved_message.model == model, f"modelが一致しない: expected={model}, actual={saved_message.model}"
    
    assert saved_message.timestamp is not None, "timestampフィールドが存在しない"
    assert isinstance(saved_message.timestamp, datetime), "timestampがdatetimeオブジェクトでない"
...
def test_retrieved_message_completeness(role, content, model):
    ...
    **Validates: Requirements 3.3**
    ...
    assert isinstance(retrieved.timestamp, datetime), "取得したメッセージのtimestampがdatetimeでない"
