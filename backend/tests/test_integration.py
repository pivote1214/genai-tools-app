...
class TestChatIntegration:
    ...
    def test_complete_chat_flow(
        self, client, mock_llm_service, mock_message_repository
    ):
        """
        完全なチャットフローの動作を検証
        要件: 1.1, 1.2, 1.3, 1.4
    ...
    def test_error_handling_in_chat_flow(
        self, client, mock_llm_service, mock_message_repository
    ):
        """
        エラーハンドリングを検証
...
class TestModelsEndpoint:
    ...
    def test_get_models_google_only(self, client):
        ...
        """
        with patch.dict(
            "os.environ", {"GEMINI_API_KEY": "test-gemini-key"}, clear=True
        ):
            response = client.get("/api/models")
...
class TestDatabaseIntegration:
    ...
    def test_message_persistence_in_chat_flow(
        self, client, mock_llm_service, mock_message_repository
    ):
        """
